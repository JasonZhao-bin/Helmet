...
/**
 * 功能：将ms转换为dw1000设备时间 function to convert microseconds to device time
 *
 */ 
...
/**
 * 功能：计算TOF距离 function to calculate and the range from given Time of Flight
 *
 */  
...
/**
 * 功能：清除计数/平均值/测距值 function to clear counts/averages/range values
 *
 */ 
...
// -------------------------------------------------------------------------------------------------------------------
/**
 * 功能：DW1000初始化 function to initialise instance structures
 *
 */ 
int instance_init(int inst_mode)
{
    ...
    //初始化dw1000设置 this initialises DW1000 and uses specified configurations from OTP/ROM
    result = dwt_initialise(DWT_LOADUCODE) ;

    //使能dw1000  LED控制 configure the GPIOs which control the leds on EVBs
    dwt_setleds(3) ; 
    ...
    //禁用接受双缓存区 disable double RX buffer
    ...
    {
        //基站回调函数设置
    	dwt_setcallbacks(tx_conf_cb, rx_ok_cb_anch, rx_to_cb_anch, rx_err_cb_anch);//基站的可以删除
    }
    else
    {
        //标签回调函数设置
    ...
    //dw1000使能PA控制 enable TX, RX state on GPIOs 6 and 5
#ifdef HW_PA
...
// -------------------------------------------------------------------------------------------------------------------
/**
 * 功能：读取设备DevID Return the Device ID register value, enables higher level validation of physical device presence
 *
 */ 
...
// -------------------------------------------------------------------------------------------------------------------
/**
 * 功能：配置信道信息  function to allow application configuration be passed into instance and affect underlying device operation
 *
 */
void instance_config(instanceConfig_t *config, sfConfig_t *sfConfig)
{
    ...
    //配置信道参数(configure the channel parameters)
    dwt_configure(&inst->configData) ;

    //降低SPI速率值低于3MHz(reduce SPI to < 3MHz)
    port_set_dw1000_slowrate(); 
    //读取OTP数据(load TX values from OTP)
    ...
    //配置TX频谱参数configure the tx spectrum parameters (power and PG delay)
	...
	//设置天线发送延迟参数(set the antenna delay, we assume that the RX is the same as TX)
...
/**
 * 功能：标签获取测距Seq  get ranging number
 *
 */
...
/**
 * 功能：基站获取测距Seq  get ranging number
 *
 */
...
/**
 * 功能：获取长计数值  get count of ranges used for calculation of lt avg
 *
 */
...
/**
 * 功能：获取距离数值 单位mm get instantaneous range
 *
 */
...
/**
 * 功能：获取距离数值 单位mm(未校正) //get instantaneous range (uncorrected)
 *
 */
...
/**
 * 功能：设置16bit短地址 set the 16-bit MAC address
 *
 */ 
...
/**
 * 功能：配置空中数据Panid和FC填充  this function configures the Frame Control and PAN ID bits
 *
 */ 
...
/**
 * 功能：控制发送长度，并发送  this function writes DW TX Frame Control, Delay TX Time and Starts Transmission
 *
 */  
...
/**
 * 功能：发送回调函数  This function is called from the (TX) interrupt handler
 *
 */  
...
/**
 * 功能：设置固定回复延迟时间 function to set the fixed reply delay time (in us)
 *
 */
...
/**
 * 功能：计算每一个距离值 calculate range for each ToF in the array, and return a mask of valid ranges
 *
 */
